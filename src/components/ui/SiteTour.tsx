...
const TOUR_KEY = 'easyprep_tour_seen_v2';
...
const STEPS: TourStep[] = [
  ...
  },
  {
    ...
    bodyHe: 'תכנן כל יום בשבוע — גרור מתכונים לכל ארוחה וקבל רשימת קניות שבועית.',
    bodyEn: 'Plan each day of the week — assign recipes to meals and get a full weekly shopping list.',
    placement: 'bottom',
  },
  {
          ...
          {pos && (
            <motion.div
              ...
              style={{
                ...
                background: '#fff',
                borderRadius: 16,
                boxShadow: '0 12px 40px rgba(26,25,24,0.18)',
                padding: '16px 18px',
            ...
            >
              {/* Arrow up */}
              <div style={{
                position: 'absolute', top: -7, right: isHe ? 'auto' : 'auto', left: 20,
                width: 14, height: 7, overflow: 'hidden',
              }}>
                <div style={{ width: 10, height: 10, background: '#fff', border: '1px solid #E0D9CE', transform: 'rotate(45deg)', marginTop: 3, marginLeft: 2 }} />
              </div>

              {/* Step indicator */}
              <div style={{ display: 'flex', gap: 4, marginBottom: 10 }}>
                {STEPS.map((_, i) => (
                  <div key={i} style={{ height: 4, borderRadius: 9999, flex: 1, background: i <= step ? '#14422d' : '#E0D9CE', transition: 'background 0.2s' }} />
                ))}
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <button
                  onClick={finish}
                  style={{ background: 'none', border: 'none', fontSize: 12, color: '#A09893', cursor: 'pointer', padding: 0 }}
                >
                  {isHe ? 'דלג' : 'Skip'}
                </button>
                <div style={{ display: 'flex', gap: 8 }}>
                  {step > 0 && (
                    <button
                      onClick={prev}
                      style={{ padding: '7px 14px', borderRadius: 9999, background: '#F0EBE3', border: 'none', fontSize: 12, fontWeight: 600, color: '#6B6560', cursor: 'pointer' }}
                    >
                      {isHe ? '→' : '←'}
                    </button>
                  )}
                  <button
                    onClick={next}
                    style={{ padding: '7px 16px', borderRadius: 9999, background: '#14422d', border: 'none', fontSize: 12, fontWeight: 600, color: '#fff', cursor: 'pointer' }}
                  >
                    {step < STEPS.length - 1
                      ? (isHe ? 'הבא ←' : 'Next →')
                      : (isHe ? 'סיום ✓' : 'Done ✓')}
